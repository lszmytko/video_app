...
import { VideoInterface } from "../../context/types";

const Video = (props: VideoInterface) => {
  const { deleteVideo, toggleModal, addToFavourite, display } =
    useGlobalContext();
  const { id, url, date, img, type, favourite, snippet, name, statistics } =
    props;
  return (
    <Col
        ...
        <Col xs="11" className="image-container-column">
          ...
          <Row className="video-info align-items-center justify-content-center">
            <Col xs="3" className="video-info-details">
              {type === "youtube" ? statistics?.likeCount : "No data"}
            </Col>
            <Col xs="3" className="video-info-details">
              {type === "youtube" ? statistics?.dislikeCount : "No data"}
            </Col>
            <Col xs="3" className="video-info-details">
              {type === "youtube" ? statistics?.viewCount : "No data"}
            </Col>
            <Col xs="3" className="video-info-details">
